...
def analyze_documentation(component_name: str, documentation_source: str = None):
    ...
    doc_content = ""
    key_terms = []
    integration_steps_summary = "No documentation content provided or found for detailed analysis."
        ...
        if documentation_source.startswith("http"): # Assume URL
            ...
            try:
                ...
                if fetch_result and 'output' in fetch_result:
                    ...
                    key_terms = [term for term in ['API', 'Integration', 'Setup', 'Configuration', 'Database', 'Authentication', 'Real-time', 'Tasks', 'Asynchronous'] if term.lower() in doc_content.lower()]
                    integration_steps_summary = "Content fetched. Further parsing needed for detailed steps."
                else:
            ...
            try:
                ...
                if read_result and 'content' in read_result:
                    ...
                    key_terms = [term for term in ['API', 'Integration', 'Setup', 'Configuration', 'Database', 'Authentication', 'Real-time', 'Tasks', 'Asynchronous'] if term.lower() in doc_content.lower()]
                    integration_steps_summary = "Content read. Further parsing needed for detailed steps."
                else:
        ...
        try:
            ...
            if search_results and 'output' in search_results:
                ...
                if urls:
                    ...
                    for url in urls:
                        try:
                            fetch_result = default_api.web_fetch(prompt=f"Get content from {url}")
                            if fetch_result and 'output' in fetch_result:
                                doc_content = fetch_result['output']
                                print(f"Documentation fetched from {url} (first 200 chars):", doc_content[:200])
                                # Basic keyword extraction from fetched content
                                key_terms = [term for term in ['API', 'Integration', 'Setup', 'Configuration', 'Database', 'Authentication', 'Real-time', 'Tasks', 'Asynchronous'] if term.lower() in doc_content.lower()]
