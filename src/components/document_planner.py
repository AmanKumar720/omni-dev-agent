...
class DocumentPlanner:
    ...
    def generate_plan(
        self, task_description: str, task_type: str = "general_development"
    ) -> TaskPlan:
        """
        ...
        if task_type == "general_development" and self._is_web_development_task(
            task_description
        ):
            task_type = "web_development"

        template = self.plan_templates.get(
            task_type, self.plan_templates["general_development"]
        )
        ...
        for i, phase_name in enumerate(template["phases"], 1):
            ...
            effort = self._estimate_effort(
                phase_name, step_descriptions, template, task_description
            )
    ...
    def _get_steps_for_phase(
        self, phase_name: str, template: Dict[str, Any]
    ) -> List[str]:
    ...
    def _get_phase_dependencies(
        self, phase_number: int, total_phases: int
    ) -> List[int]:
